...
export default function Calendar({ userId, refreshKey }: Props) {
    ...
    <Card className="h-full flex flex-col px-1 pb-4 bg-[#F9F1F1] overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-7 gap-4 flex-wrap">
        ...
        <div className="flex gap-2 flex-shrink-0">
          <button
            className={`px-3 py-1 rounded-xl font-semibold text-sm transition-colors ${
              view === "month"
                ? "bg-[#FADEDE] text-[#3F3131]"
                : "bg-[#E7D7D7] text-[#3F3131] hover:bg-[#FADEDE]"
            }`}
        ...
        <div className="flex items-center gap-2 ml-4 flex-1 justify-end">
          ...
          <span className="text-lg font-bold text-[#3F3131] font-(family-name:--font-IBMPlexSans)">
            {view === "month"
              ...
              ? `${getWeekDates(
                  current
                )[0].toLocaleDateString()} - ${getWeekDates(
                  current
      ...
      {view === "month" && (
        <div className="flex flex-col flex-1 h-full">
          ...
          {getMonthMatrix(current.getFullYear(), current.getMonth()).map(
            (week, i, weeksArr) =>
              week.some((date) => date.getMonth() === current.getMonth()) ? (
                <div key={i} className="flex flex-1 min-h-0 w-full">
                  {week.map((date, j) => {
                    ...
                    const isFirstVisibleWeek =
                      i ===
                      weeksArr.findIndex((w) =>
                        w.some((d) => d.getMonth() === current.getMonth())
                      );
                    const isLastVisibleWeek =
                      i ===
                      weeksArr
                        .map((w) =>
                          w.some((d) => d.getMonth() === current.getMonth())
                    ...
                    let roundedClass = "";
                    if (isFirstVisibleWeek && isFirstDay)
                      roundedClass += " rounded-tl-xl";
                    if (isFirstVisibleWeek && isLastDay)
                      roundedClass += " rounded-tr-xl";
                    if (isLastVisibleWeek && isFirstDay)
                      roundedClass += " rounded-bl-xl";
                    if (isLastVisibleWeek && isLastDay)
                    ...
                    return (
                      <Card
                        ...
                        className={`flex flex-col items-start p-4 border ${
                          isCurrentMonth
                            ? "border-[#E8E3E3] bg-white"
                            : "border-[#C8B7B7] bg-[#F0E4E4] opacity-40"
                        } rounded-none flex-1 h-full min-h-0 min-w-0 w-full${roundedClass}`}
                        onMouseEnter={(e) => {
                      ...
                      >
                        <div className="w-full flex justify-center items-start mb-[2px]">
                          <span className="text-xs font-bold text-[#3F3131] font-(family-name:--font-IBMPlexSans)">
                            <span
                              className={`rounded-full w-7 h-7 flex items-center justify-center mx-auto pointer-events-auto ${
                                isToday(date)
                                  ? "bg-blue-500 text-white"
                                  : "bg-gray-200 text-[#3F3131]"
                              }`}
                            >
                              {date.getDate()}
                            </span>
                        ...
                        <div className="mt-3 flex flex-col gap-1 w-full">
                          {(() => {
                            ...
                            dayTasks.forEach((task) => {
                              if (!classMap.has(task.classId))
                                classMap.set(task.classId, []);
                            ...
                            return (
                              <>
                                <span
                                  key={firstTask._id}
                                  className={`block text-xs truncate px-1 py-0.5 rounded font-medium ${
                                    firstTask.isComplete
          ...
          <Card className="min-h-[200px] flex flex-col items-start p-4 border border-[#E7D7D7] bg-[#F9F1F1]">
            ...
            <div className="mt-3 flex flex-col gap-2 w-full">
              {(() => {
                ...
                if (dayTasks.length === 0) {
                  return (
                    <span className="text-[#654545] text-base font-medium font-(family-name:--font-IBMPlexSans)">
                ...
                return dayTasks.map((task) => (
                  <span
                    key={task._id}
                    className={`block text-sm truncate px-2 py-1 rounded font-medium ${
                      task.isComplete
                        ? "bg-green-100 text-green-700"
                        : "bg-[#FADEDE] text-[#3F3131]"
                    }`}
          ...
          return (
            ...
            >
              <div className="font-bold text-[#3F3131] mb-2 text-base">
                {hoverDate
                  ? `Tasks for ${hoverDate.toLocaleDateString(undefined, {
                      month: "long",
                      day: "numeric",
                    })}`
                  : "Tasks"}
              ...
              <div className="text-[#654545] text-xs mb-2 font-medium">
                You currently have {hoverTasks.length} task
                {hoverTasks.length !== 1 ? "s" : ""}
              </div>
              <div className="flex flex-col gap-3">
                {hoverTasks.map((task) => (
                  <div
                    key={task._id}
                    className={`flex flex-col gap-1 py-2 px-2 rounded ${
                      task.isComplete ? "bg-green-200" : "bg-[#F5D7B8]"
                    }`}
                  >
                    <span className="font-semibold text-[#3F3131]">
                      {task.name}
                    </span>
                    <span className="text-xs text-[#654545]">
                      Due:{" "}
                    ...
                    <span
                      className={`text-xs font-bold ${
                        task.isComplete ? "text-green-700" : "text-[#3F3131]"
                      }`}
                    >
